const CACHE='mishkat-hadith-screen-v07-20260822';
...
self.addEventListener('install',e=>e.waitUntil((async()=>{const c=await caches.open(CACHE);for(const url of ASSETS){const r=await fetch(url,{cache:'reload'});if(!r.ok)throw new Error(`Precache failed ${url}: ${r.status}`);await c.put(url,r)}const keys=await caches.keys();if(keys.some(k=>k.startsWith('mishkat-hadith-screen-v06-')))await self.skipWaiting()})()));
